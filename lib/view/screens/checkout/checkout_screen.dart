...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    ...
    if(_isLoggedIn) {
      ...
      Get.find<LocationController>().getAddressList().then((value) => addressModel =  Get.find<LocationController>().addressList.length==0
          ? Get.find<LocationController>().getUserAddress()
          : Get.find<LocationController>().addressList[0]   );
                        ...
                        orderController.orderType != 'take_away' ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          GestureDetector(
                            child: AddressWidget(
                              ...
                              fromAddress: false, fromCheckout: false,
                            ),
                            onTap: popUpAdd
                          ),
                          SizedBox(height: Dimensions.PADDING_SIZE_LARGE),
                ...
                Container(
                  ...
                  child: !orderController.isLoading ? CustomButton(buttonText: 'confirm_order'.tr, onPressed: () {
                    ...
                    if(_orderAmount < restController.restaurant.minimumOrder) {
                      ...
                      if(orderController.paymentMethodIndex != 0)
                      Get.dialog(ConfirmationDialog(
                                icon: Images.warning, description: 'Order once placed can\'t be canceled. \nDo you want to proceed ?' , onYesPressed: () {
                          ...
                          paymentMethod: _isDigitalPaymentActive ? orderController.paymentMethodIndex == 0 ? 'digital_payment'
                          ...
                          address: addressModel.address, latitude: addressModel.latitude, longitude: addressModel.longitude, addressType: addressModel.addressType,
                          contactPersonName: addressModel.contactPersonName ?? '${Get.find<UserController>().userInfoModel.fName} '
                              '${Get.find<UserController>().userInfoModel.lName}',
                          contactPersonNumber: addressModel.contactPersonNumber ?? Get.find<UserController>().userInfoModel.phone,
  ...
  Widget setupAlertDialogContainer(BuildContext context) {
    return _isLoggedIn ? GetBuilder<LocationController>(builder: (locationController) {
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Dismissible(
                  ...
                  child: AddressWidget(
                    address: locationController.addressList[index], fromAddress: false, fromCheckout: false,
                    onTap: () {
                      setState(() {
                        addressModel= locationController.addressList[index];
                      });
      ...
      ) : Text( 'no_saved_address_found'.tr) : Center(child: CircularProgressIndicator());
    }) : NotLoggedInScreen();
  }
